...
class BattleModel:
    ...
    Attributes:
        combatants (List[Meal]): The list of meals in the collection.
    ...
    def get_battle_score(self, combatant: Meal) -> float:
        """
        Calculates, logs, and gets the battle score of a combatant.

        Args:
            combatant (Meal): the meal to calculate the battle score of.
        """
    ...
    def get_combatants(self) -> List[Meal]:
        """
        Returns a list of all combatants in the combatants list.
        """
    ...
    def prep_combatant(self, combatant_data: Meal):
        """
        Adds combatant data to a meal.

        Args:
            combatant_data (Meal): combatants for meal to battle.

        Raises:
            ValueError: If the meal already has a full list of combatants.
        """
        if len(self.combatants) >= 2:
